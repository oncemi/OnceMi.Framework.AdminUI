...
<script>
import { GET_VIEW_LIST, DELETE_VIEW_ITEM, POST_VIEW_ITEM, PUT_VIEW_ITEM } from "@/services/api";
...
export default {
  ...
  methods: {
    ...
    load() {
      this.loading = true;
      request(GET_VIEW_LIST, METHOD.GET, {
        page: this.page,
        size: this.pageSize,
    ...
    save() {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          if (values.id > 0) {
            // 修改 e.g.
            request(PUT_VIEW_ITEM, METHOD.PUT, values)
              .then((result) => {
            ...
            request(POST_VIEW_ITEM, METHOD.POST, values)
              .then((result) => {
                if (result.data.code != 0) {
                  this.$refs.createModal.loading = false;
                  return;
