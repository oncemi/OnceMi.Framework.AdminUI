...
<script>
import { GET_MENU_LIST, DELETE_MENU_ITEM, POST_MENU_ITEM, PUT_MENU_ITEM } from "@/services/api";
...
export default {
  ...
  methods: {
    load() {
      this.loading = true;
      request(GET_MENU_LIST, METHOD.GET, {
        page: this.page,
        size: this.pageSize,
    ...
    save() {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          if (values.type == 2 && (!values.apiId || values.apiId == 0)) {
            this.$message.warning("当菜单类型为接口时，接口不能为空");
            this.$refs.createModal.loading = false;
            return;
          }
          if ((values.type == 1 || values.type == 3) && (!values.viewId || values.viewId == 0)) {
            this.$message.warning("当菜单类型为视图或分组时，视图不能为空");
            this.$refs.createModal.loading = false;
            return;
          }
          if (values.id > 0) {
            // 修改 e.g.
            request(PUT_MENU_ITEM, METHOD.PUT, values)
              .then((result) => {
                if (result.data.code != 0) {
                  this.$refs.createModal.loading = false;
                  return;
            ...
            request(POST_MENU_ITEM, METHOD.POST, values)
              .then((result) => {
                if (result.data.code != 0) {
                  this.$refs.createModal.loading = false;
                  return;
    ...
    deleteWithConfirm(records) {
      ...
      this.$confirm({
        ...
        okType: "danger",
        onOk() {
          return request(DELETE_MENU_ITEM, METHOD.DELETE, delArgs)
            .then((result) => {
              if (result.data.code != 0) {
                return;
              }
